...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
  	if (this.isEmpty) acc else
  	if (p(this.head)) this.tail.filterAcc(p, acc incl this.head)
  	else this.tail.filterAcc(p, acc )
  }
  ...
  def tail = if (left.isEmpty) right else new NonEmpty(elem, left.tail, right)
	def mostRetweeted: Tweet = {
		if (left.isEmpty & right.isEmpty)  elem
			...
			val rightnode = right.mostRetweeted
			if (elem.retweets>rightnode.retweets) elem
			else rightnode
		}else if (right.isEmpty){
			val leftnode = left.mostRetweeted
			if (elem.retweets>leftnode.retweets) elem
			else  leftnode
		}else{
			val leftnode = left.mostRetweeted
			val rightnode = right.mostRetweeted
			if ((elem.retweets >leftnode.retweets)&(elem.retweets>rightnode.retweets)) elem
			else if (leftnode.retweets>rightnode.retweets)  leftnode
			else  rightnode
		}
	}
	def descendingByRetweet: TweetList = {
		def desec(acc:TweetSet) : TweetList = {
			if (acc.isEmpty)  new Nil
			else {
				val node = acc.mostRetweeted
				 new Cons(node,desec(acc.remove(node)))
			}
		}
		desec(this)
	}
...
object GoogleVsApple {
  ...
  lazy val testing: TweetSet = TweetReader.allTweets
...
object Main extends App {
  ...
  println ("start")
 GoogleVsApple.testing foreach println
